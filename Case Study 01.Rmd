...
## Load Data
```{r}
#setwd("C:/Personal/SMU/DDS6306/Unit8-9")
...
```{r}
plot_missing(beer_brewery_merged_df, title = "Missing Value State Post Merge of Data Sets", ggtheme = theme_clean())
...
beer_brewery_IPAandALE <- filter(beer_brewery_merged_df,grepl('IPA|Ale',Style))
beer_brewery_IPAandALE <- beer_brewery_IPAandALE[,-8]

# Create a new dataframe that only holds the ABV,IBU, and beer style.

beer_brewery_IPAandALE_subset <- select(beer_brewery_IPAandALE,ABV,IBU,Style)
beer_brewery_IPAandALE_subset$Style <- as.character(beer_brewery_IPAandALE_subset$Style)
...
set.seed(1234)
splitpale = .70
samplesplit <- sample(1:dim(beer_brewery_IPAandALE_subset)[1],round(splitpale * dim(beer_brewery_IPAandALE_subset)[1]))
```
...
accuplot <- plotly::ggplotly(accuplot)
...
Brew_Cities = group_by(beer_brewery_merged_df, City, State)
Brew_Cities = summarize(Brew_Cities,count = n())
data <- world.cities %>% filter(country.etc=="USA")
try(data <- rename(data, City = name))

Brew_Cities <- right_join(data, Brew_Cities, by="City")
Brew_Cities <- Brew_Cities %>% na.omit()
...
fig <- plot_geo(Brew_Cities, locationmode='USA -states', sizes =c(1,250))
fig <- fig %>% add_markers(x = ~long, y = ~lat, size = ~count, color = ~count, hoverinfo = "text",
    text = ~paste(City, State, paste("Breweries:", count), sep = "<br />"),
    size = I(8), 
  )
fig <- fig %>% colorbar(title = "Breweries")
fig <- fig %>% layout(
    title = 'US Breweries', geo = g
  )
