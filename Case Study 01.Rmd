...
breweries_df <- read.csv("./data/Breweries.csv", header = T, sep = ",")

#TODO look at using data.table versus dataframe
```
## Inspect the data
...
#TODO add plot to show any duplicates
brewery_dupes <- as.character(breweries_df[which(duplicated(as.character(breweries_df$Name))), "Name"]) # Check for duplicates
breweries_df %>% filter(Name == "Blackrocks Brewery")      # Brew_ID, State (MI, MA)
breweries_df %>% filter(Name == "Summit Brewing Company")  # Brew_ID, City (St. Paul, St Paul, same city?)
breweries_df %>% filter(Name == "Otter Creek Brewing")     # Brew_ID, City (Waterbury, Middlebury)
breweries_df %>% filter(Name == "Sly Fox Brewing Company") # Brew_ID, City (Phoenixville, Pottstown)
breweries_df %>% filter(Name == "Blue Mountain Brewery")   # Brew_ID, City (Afton, Arrington)
breweries_df %>% filter(Name == "Lucette Brewing Company") # Brew_ID, City (Menominee, Menominie, same city?)
breweries_df %>% filter(Name == "Oskar Blues Brewery")     # Brew_ID, City (Longmont, Lyons)

breweries_df$City <- str_replace_all(breweries_df$City, "St. Paul", "St Paul") ## Change City St. Paul to St Paul
breweries_df$City <- str_replace_all(breweries_df$City, "Menominee", "Menominie") ## Change City Menominee to Menominie
...
map_df <- left_join(states_df, brewery_count, by = "region")

#TODO try using ggplotly to see if it helps make it pop
ggplot(map_df, aes(x = long, y = lat), color = white) +
  ggtitle("Breweries per State") +
  geom_polygon(aes(fill = Breweries, group = group), stat = "identity") +
  geom_path() +
  scale_fill_gradientn(colours = topo.colors(5)) +
  #geom_text(aes(label=region), hjust=0.5, vjust=0.5, size=3) +
  #geom_text(aes(label = brewery_count$Breweries, )) +
  coord_map() + 
...
treemap(brewery_count, 
        index = c("State", "Breweries"),
        vSize = "Breweries",
        type = "index", 
        palette = "Blues",
...
```{r}
beer_brewery_merged_df <- full_join(beers_df, breweries_df, by = "Brewery_id")
...
beer_brewery_merged_df <- beer_brewery_merged_df %>%  mutate(IBU = ifelse(is.na(IBU) | is.na(ABV),0, IBU))
aggr(beer_brewery_merged_df, prop=FALSE, numbers=TRUE)

ibu_na <- beer_brewery_merged_df %>% filter(is.na(IBU))
abv_na <- beer_brewery_merged_df %>% filter(is.na(ABV))
ibu_abv_na <- beer_brewery_merged_df %>% filter(is.na(ABV) & is.na(IBU))
...
ibu_median <- beer_brewery_merged_df %>% group_by(State) %>% summarise_each(funs(median), median = IBU) %>% arrange(median)
head(ibu_median,5)
tail(ibu_median,5)

abv_median <- beer_brewery_merged_df %>% na.omit() %>% group_by(State) %>% summarise_each(funs(median), median = ABV) %>% arrange(median)
head(abv_median,5)
tail(ibu_median,5)
...
```{r}
ibu_top <- beer_brewery_merged_df %>% top_n(5, IBU)
ibu_max <- max(beer_brewery_merged_df$IBU,na.rm=TRUE)
ibu_max_state <- beer_brewery_merged_df %>% filter(IBU==ibu_max) %>% select(State,IBU)
ibu_max_state
ibu_max_beer <- beer_brewery_merged_df[which.max(beer_brewery_merged_df$IBU),]
ibu_max_beer

abv_top <- beer_brewery_merged_df %>% top_n(5, ABV)
abv_max <- max(beer_brewery_merged_df$ABV,na.rm=TRUE)
abv_max_state <- beer_brewery_merged_df %>% filter(ABV==abv_max) %>% select(State,ABV)
abv_max_state
abv_max_beer <- beer_brewery_merged_df[which.max(beer_brewery_merged_df$ABV),]
abv_max_beer

# plot IBU top by state and beer
ggplot(ibu_top, aes(x=State, y=IBU, fill=State)) + geom_bar(stat = "identity") + ggtitle("Top 5 IBU by States")+
...
ggplot(abv_top, aes(x=State, y=ABV, fill=State)) + geom_bar(stat = "identity") + ggtitle("Top 5 ABV by States")+
...
beer_brewery_merged_df %>% ggplot(aes(
  y = ABV,
  x = Beer_Type,
  fill = as.factor(Ounces)
)) + geom_boxplot() +
  theme(axis.text.y = element_text(size=rel(0.8), angle=90)) +
  labs(x="Beer Type",y="ABV") + 
...
##   Histogram
beer_brewery_merged_df %>% ggplot(aes(x = ABV)) + geom_histogram(fill = "blue", color =
"black") + theme(axis.text.y = element_text(size=rel(0.8), angle=90)) +
  labs(x="",y="ABV") 
...
beer_brewery_merged_df %>% na.omit() %>% ggplot(aes(x=IBU, y=ABV, color=Beer_Type)) + geom_point() + theme(axis.text.y = element_text(size=rel(0.8), angle=90)) +
  labs(x="State",y="IBU") + ggtitle("ABV vs IBU") + geom_smooth(method = lm)
...
beer_brewery_merged_df %>% ggplot(aes(x=ABV, y=IBU)) + 
  geom_point(aes(color=State), size=1, position="jitter") + 
  geom_smooth(method = lm) + 
  theme(axis.text.y = element_text(size=rel(0.8), angle=90)) +
  labs(x="ABV",y="IBU") +
...
cor.test(beer_brewery_merged_df$IBU, beer_brewery_merged_df$ABV)
ibu_abv_mod <- lm(beer_brewery_merged_df$IBU ~ beer_brewery_merged_df$ABV)
summary(ibu_abv_mod)
